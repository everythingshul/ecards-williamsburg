...
// Recent Actions ('audit') is deliberately NOT in this list — see
// routes/audit.js, which gates its whole router on requireRole('super_admin')
// directly instead of going through requirePermission()/this list at all.
// It used to be a grantable-but-default-denied resource here (same
...
// /stores/:id/impersonate) stays denied for org_admin/staff until a
// specific user is explicitly granted it via Users & Permissions — it hands
// whoever has it a real, unaudited-from-the-shul's-side session as that
// shul/store, so it should never be something org_admin/staff get silently
